...
class NodoArbolSintactico:
    def __init__(self, valor, nonterminal):
        self.valor = valor
        self.nonterminal = nonterminal
        self.izquierda = None
        self.derecha = None

    def insertar(self, valor, nonterminal):
        if nonterminal < self.nonterminal:
            if self.izquierda is None:
                self.izquierda = NodoArbolSintactico(valor, nonterminal)
            else:
                self.izquierda.insertar(valor, nonterminal)
        else:
            if self.derecha is None:
                self.derecha = NodoArbolSintactico(valor, nonterminal)
            else:
                self.derecha.insertar(valor, nonterminal)

    def _str_(self):
        return str(self.valor)

    def postOrder(self, lado=None, nivel=0):
        if self.izquierda:
            self.izquierda.postOrder("izquierda", nivel + 1)
        if self.derecha:
            self.derecha.postOrder("derecha", nivel + 1)
        lado_actual = lado if lado else "raíz"
        print(f"Nivel {nivel}: {str(self.valor)} ({lado_actual})")
...
while True:
    try:
        ...
        def miParser():
            ...
            scope = 0
            raiz = NodoArbolSintactico("Programa", 0)
            nodo_actual = raiz  # Nodo actual
            while True:
                ...
                if x == tok.type and x == "eof":
                    ...
                    input("Presiona Enter para imprimir el arbol sintactico...")
                    raiz.postOrder()
                    input("Presiona Enter para imprimir la tabla de simbolos...")
                    ...
                    if x not in tokens:  # es no terminal
                        ...
                        print(x)
                        print(tok.type)
                        nodo_actual.insertar(tok.type, x)
